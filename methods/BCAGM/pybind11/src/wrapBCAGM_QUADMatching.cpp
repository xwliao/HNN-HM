...
py::tuple wrapBCAGM_QUADMatching(
        ...
        int subroutine, int adapt)
{
    ...
    if (Xbuf.size != 2 * N1 * N2)
        throw std::runtime_error("X.size must be 2 * N1 * N2");
    ...
    if (adapt == 0) {
        bcagm_quad(pIndH3, pValH3, Nt3,
                   pX, N1, N2,
                   subroutine,
                   pXout, pObjs, &nIter);
    } else {
        adapt_bcagm_quad(pIndH3, pValH3, Nt3,
                         pX, N1, N2,
                         subroutine, adapt,
                         pXout, pObjs, &nIter);
    }
